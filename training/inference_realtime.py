                    ...
                    if not subscription.queue.empty():
                        ...
                        if len(sensor_values) == 8:
                            ...
                            if prediction is not None:
                                ...
                                # Display prediction every 10 samples
                                if sample_count % 10 == 0:
                                    elapsed = time.time() - start_time
                                    fps = sample_count / elapsed if elapsed > 0 else 0

                                    print(f"\n[Sample {sample_count}] FPS: {fps:.1f}")
                                    print("Predictions:")
                                    for i, name in enumerate(self.finger_names):
                                        print(f"  {name:<12}: {prediction[i]:.3f}")
...
def main():
    ...
    port = "MOCK"  # Change to actual port like "COM3" or "/dev/ttyUSB0"
    baudrate = 115200
